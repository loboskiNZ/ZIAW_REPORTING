-- V1__ziaw_governance_reporting.sql

-- PRE-REQUISITE: Table `workspace` must exist.
-- Including Mock Creation IF NOT EXISTS for independent testing in this Docker env.
...
-- 0b) Mock Artifact Table (For V1 Skeleton)
...
-- 0c) Mock Review Finding Table (For V1 Skeleton)
CREATE TABLE IF NOT EXISTS review_finding (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  workspace_id BIGINT NOT NULL,
  content TEXT,
  severity ENUM('LOW', 'MEDIUM', 'HIGH', 'CRITICAL') NOT NULL,
  status ENUM('DRAFT', 'REVIEW_PENDING', 'CONFIRMED', 'REJECTED', 'RESOLVED') DEFAULT 'DRAFT',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (workspace_id) REFERENCES workspace(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
